...
# --- 2. LOGIN Y ROLES (VERSIÓN RESISTENTE) ---
if "user" not in st.session_state:
    ...
    with st.form("login"):
        ...
        if st.form_submit_button("Entrar"):
            # Verificamos si existe la sección 'users' y el usuario en Secrets
            if "users" in st.secrets and u in st.secrets["users"]:
                if p == st.secrets["users"][u]:
                    st.session_state["user"] = u
                    
                    # PARACAÍDAS: Si falla la lectura de roles, asigna 'user' y no da error
                    try:
                ...
                st.error("Usuario no encontrado en la configuración.")
    st.stop()
